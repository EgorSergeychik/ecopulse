...
import { download as downloadOsmRoute, downloadWbt as downloadWbtRoute } from '@/routes/webots';
...
const downloadingOsm = ref(false);
const downloadingWbt = ref(false);
...
const downloadOsm = () => {
    ...
    fetchAndDownload(
        downloadOsmRoute.url({ query: bboxQuery() }),
        'export.osm',
        downloadingOsm,
    );
};
...
const downloadWbt = () => {
    ...
    fetchAndDownload(
        downloadWbtRoute.url({ query: bboxQuery() }),
        'export.wbt',
        downloadingWbt,
    );
};
        ...
        <section class="grid gap-6 xl:grid-cols-4">
            ...
            <div class="flex flex-col gap-4">
                ...
                <div class="rounded-xl border bg-card p-4 shadow-sm">
                    <p v-if="!bbox" class="mb-3 text-sm text-muted-foreground">
                    ...
                    <div class="flex flex-col gap-2">
                        <Button
                            class="w-full gap-2"
                            :disabled="downloadingOsm || downloadingWbt"
                            @click="downloadWbt"
                        >
                            <Download class="h-4 w-4" />
                            {{ t('pages.osm_export.download_wbt') }}
                        </Button>
                        ...
                        >
                            <Download class="h-4 w-4" />
                            {{ t('pages.osm_export.download_osm') }}
                        </Button>
                    </div>

                    <p v-if="validationError" class="mt-2 text-sm text-destructive">
                        {{ validationError }}
                    </p>
                </div>
            </div>
